...
class Writer(object):
    ...
    def __init__(self, path, flush_sec=10):
        if osp.isdir(path):
            path = osp.join(path, dt.now().strftime(
    ...
    def _init_table_meta(self):
        self._table_meta = Table('__ps__table_meta', self._metadata,
                                 ...
                                 Column('content_type', types.String,
                                        nullable=False),
    ...
    def _create_table(self, name,  content_type, **kwargs):
        ...
        try:
            self._execute(self._table_meta.insert().values(
                name=name, content_type=content_type.__name__))
    ...
    # Extra add methods
    from ._extra_writer_adds import add_plot
