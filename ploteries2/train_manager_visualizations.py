...
import numpy as np
from .figure_managers import HistogramsManager
...
class HistogramsViz(_ScalarViz):
    def __init__(self, name, fxn, min=-np.inf, max=np.inf, trace_kwargs=None, layout_kwargs=None, **hg_kwargs):
        ...
        self.add_kwargs = {'trace_kwargs': trace_kwargs,
    ...
    def update(self, batch, prediction, loss, timing, extra):
        ...
        self.val = HistogramsManager._compute_histogram(dat, **self.hg_kwargs)

    def get(self): return self.val  # super().get().view(-1)
...
class HistogramsAccumViz(_AccumViz):
    ...
    def write_epoch(self, writer, k_sample, name=None, **name_formatters):
        ...
        writer.add_histograms(
            name, [val], k_sample, bin_centers=bin_centers, compute_histogram=False,
            **self.viz.add_kwargs)
